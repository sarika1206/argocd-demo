	...
	stage ('Deploy_K8S') {
	    ...
	    stage ('Deploy into preview env'){
			...
			AWS_ACCOUNT="738507247612"
			 REGION="us-west-2"
			 CONTAINER="k8s-debian-test"
argocd app create $JOB_BASE_NAME --repo https://github.com/sarika1206/argocd-dome-deploy.git --revision HEAD --path e2e --dest-namespace argocd-prod --dest-server https://kubernetes.default.svc
